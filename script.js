...
function addTask(bordId){
    ...
    if(taskText === "")return;
    const now = new Date();
    const formattedDateTime = `${String(now.getDate()).padStart(2, '0')}-${String(now.getMonth()+1).padStart(2, '0')}-${now.getFullYear()} ${((now.getHours()%12)||12).toString().padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}:${String(now.getSeconds()).padStart(2,'0')} ${now.getHours()>=12 ? 'PM' : 'AM'}`;
...
function CreateNewElement(newTask,newDate){
    ...
    Element.classList.add('task', 'w-full', 'h-auto', 'bg-sky-500', 'text-gray-900', 'mt-3', 'rounded', 'text-justify', 'p-4', 'cursor-pointer');
    Element.innerHTML = `<span class="font-bold">${newTask}</span><br><small class="text-end block font-semibold">${newDate}</small>`;
...
function editTask(){
    if(rightClickCard !== null){
        const newEditedTask = prompt("Edit task-",rightClickCard.textContent);  
        if(newEditedTask !== "" )
        {
            rightClickCard.textContent = newEditedTask;
        }
    }
    updateTask();
}
